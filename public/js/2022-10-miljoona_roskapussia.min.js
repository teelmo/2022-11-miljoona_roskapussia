...
function App() {
  ...
  var createChart = function createChart(data) {
    ...
    var maxRadius = 6;
    var max_x = Math.max.apply(Math, _toConsumableArray(data.map(function (el) {
      return el.x;
    })));
    var max_y = Math.max.apply(Math, _toConsumableArray(data.map(function (el) {
      return el.y;
    })));
    var min_x = Math.min.apply(Math, _toConsumableArray(data.map(function (el) {
      return el.x;
    })));
    var min_y = Math.min.apply(Math, _toConsumableArray(data.map(function (el) {
      return el.y;
    })));
    ...
    var svg = src_select('.map_container').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', "translate(".concat(width / 2, " ,").concat(height / 2, ")"));
    var color = ordinal(category10).domain(range(m));

    // The largest node for each cluster.
    var clusters = new Array(m);
    var nodes = data.map(function (el) {
      var i = parseInt(el.group, 10);
      var r = Math.max(4, Math.sqrt((i + 1) / m) * -Math.log(Math.random()) * maxRadius);
    ...
    .attr('cx', function (d) {
      ...
      src_selectAll('circle').style('opacity', 0.3);
      src_select(event.target).style('opacity', 1);
      src_select('.tooltip').style('left', "".concat(event.offsetX + 10, "px")).style('top', "".concat(event.offsetY + 10, "px")).style('display', 'inline').style('opacity', 1).html(d.name);
    }).on('mouseout', function () {
      src_selectAll('circle').style('opacity', 1);
      src_select('.tooltip').style('opacity', 0).style('display', 'none');
    });
  ...
  (0,react.useEffect)(function () {
    var getDataPath = function getDataPath() {
      ...
      if (true) return 'https://lusi-dataviz.ylestatic.fi/2022-10-miljoona_roskapussia/assets/data/data.csv';
      return 'assets/data/data.csv';
    };
    try {
      fetch(getDataPath()).then(function (response) {
        return response.text();
      }).then(function (body) {
        return createChart(cleanData(helpers_CSVtoJSON(body)));
      });
    } catch (error) {
      console.error(error);
    }
  ...
  return /*#__PURE__*/react.createElement("div", {
    ...
    min: "1",
    max: "30",
    placeholder: "Sy\xF6t\xE4 pussien m\xE4\xE4r\xE4"
  }))), /*#__PURE__*/react.createElement("div", {
    className: "input_wrapper"
  }, /*#__PURE__*/react.createElement("label", {
    ...
    type: "button",
    value: "L\xE4het\xE4 kartalle"
  })))), /*#__PURE__*/react.createElement("div", {
    className: "container"
  }, /*#__PURE__*/react.createElement("h3", null, "Nykytilanne kartalla"), /*#__PURE__*/react.createElement("div", {
    className: "input_container"
  }, /*#__PURE__*/react.createElement("div", {
    className: "input_wrapper"
  }, /*#__PURE__*/react.createElement("label", {
...
var appName = '2022-10-miljoona_roskapussia';
